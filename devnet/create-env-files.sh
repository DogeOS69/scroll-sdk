...
get_service_configmap_variables() {
    ...
    case "$service_name" in
        ...
        l1-explorer)
            ...
            ;;
        l2-bootnode)
            echo "CHAIN_ID_L2:CHAIN_ID L1_RPC_ENDPOINT:L2GETH_L1_ENDPOINT L2_GETH_STATIC_PEERS:L2GETH_PEER_LIST L1_CONTRACT_DEPLOYMENT_BLOCK:L2GETH_L1_CONTRACT_DEPLOYMENT_BLOCK BEACON_RPC_ENDPOINT:L2GETH_DA_BLOB_BEACON_NODE"
            ;;
        l2-rpc)
            echo "CHAIN_ID_L2:CHAIN_ID L1_RPC_ENDPOINT:L2GETH_L1_ENDPOINT L1_CONTRACT_DEPLOYMENT_BLOCK:L2GETH_L1_CONTRACT_DEPLOYMENT_BLOCK L2_GETH_STATIC_PEERS:L2GETH_PEER_LIST BEACON_RPC_ENDPOINT:L2GETH_DA_BLOB_BEACON_NODE"
            ;;
        l2-sequencer)
            echo "CHAIN_ID_L2:CHAIN_ID L1_RPC_ENDPOINT:L2GETH_L1_ENDPOINT L2GETH_SIGNER_ADDRESS:L2GETH_SIGNER_ADDRESS L1_CONTRACT_DEPLOYMENT_BLOCK:L2GETH_L1_CONTRACT_DEPLOYMENT_BLOCK L2GETH_KEYSTORE:L2GETH_KEYSTORE L2GETH_PASSWORD:L2GETH_PASSWORD L2GETH_NODEKEY:L2GETH_NODEKEY L2_GETH_STATIC_PEERS:L2GETH_PEER_LIST BEACON_RPC_ENDPOINT:L2GETH_DA_BLOB_BEACON_NODE"
            ;;
        rollup-relayer)
...
extract_from_config_toml() {
  ...
  extract_and_export() {
    ...
    value=$(grep -E "^${source_var} =" "$toml_file" | head -n 1 | sed 's/ *= */=/' | cut -d'=' -f2-)
    ...
    value=$(echo $value | sed -E 's/^([0-9_]+)$/\"\1\"/')
...
services_configmap=("admin-system-backend" "balance-checker" "bridge-history-api" "bridge-history-fetcher" "blockscout" "chain-monitor" "contracts" "coordinator" "gas-oracle" "l1-devnet" "l1-explorer" "l2-bootnode" "l2-rpc" "l2-sequencer" "rollup-relayer")
